...
const FoodDetails: React.FC = () => {
  ...
  useEffect(() => {
    async function loadFood(): Promise<void> {
      ...
      const extrasSerialized = response.data.extras.map(extra => {
        return {
          ...extra,
          quantity: 0,
        };
  ...
  function handleIncrementExtra(id: number): void {
    setExtras(state => {
      const newExtras = [...state].map(extra => {
        if (extra.id === id) {
          return {
            ...extra,
  ...
  const toggleFavorite = useCallback(async () => {
    if (isFavorite) {
      ...
      await api.post('favorites', data).then(() => {
        setIsFavorite(true);
      });
    }
  }, [isFavorite, food]);
  ...
  const cartTotal = useMemo(() => {
    ...
    const extrasTotalValue =
      extras.reduce((sum, extra) => sum + extra.value * extra.quantity, 0) *
      foodQuantity;

    const totalValue = foodTotalValue + extrasTotalValue;
